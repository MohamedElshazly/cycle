...
export function CheckinModal({ cycleId, userId, isOpen, onClose, onSuccess }: CheckinModalProps) {
	const [step, setStep] = useState<Step>(1)
	const [selected, setSelected] = useState<number | string | null>(null)

	const { setValue, watch, getValues, reset } = useForm<CheckinForm>({
		defaultValues: { didTheThing: null, generalFeeling: null, note: '' },
	})

	const { submit, submitting } = useCheckins(cycleId)
	...
	const handleSave = useCallback(async () => {
		...
		if (!data.didTheThing || !data.generalFeeling) return
		try {
	...
	}, [userId, getValues, submit, reset, onSuccess])
	...
	return (
		<div className="fixed inset-0 z-50" style={{ color: 'var(--text-primary)' }}>
			...
			<div
				...
				onClick={step === 'done' ? undefined : handleClose}
			/>
			...
			>
				<div className="w-full h-full flex flex-col items-center justify-center pointer-events-auto" style={{ background: 'var(--background)' }}>
					{/* Ambient glow */}
					<div
					...
					{step !== 'done' && (
						<button
							onClick={handleClose}
							className="fixed top-8 right-8 checkin-close-btn w-10 h-10 flex items-center justify-center rounded-full transition-colors duration-150 z-[60] pointer-events-auto"
